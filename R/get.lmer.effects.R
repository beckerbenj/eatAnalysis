...
#'@param quick Logical: Sometimes, default method for computing \eqn{R^2} by calling \code{\link[MuMIn]{r.squaredGLMM}} is very time-consuming. Using \code{quick = TRUE} forces the function to choose an alternative method which, however, is not available when \code{lmerTest} is attached. If \code{lmerTest} is attached, no \eqn{R^2} is computed if \code{quick = TRUE}.
#'
...
#'}
#'@export
get.lmer.effects <- function ( lmerObj , bootMerObj = NULL, conf = .95, saveData = FALSE, quick=FALSE) {
     ### untere Zeile: man muss diesen komplizierten Weg waehlen, weil sonst das Objekt 'model' eine laenge > 1 hat, wenn das uebergebene Objekt 'lmerObj' etwa 'mod[[1]]' heisst
             ...
             # checkForPackage (namePackage = "reshape", targetPackage = "eatGet")### Beispiel in c:\diskdrv\Winword\Psycho\IQB\Dropbox\Literatur\R_help\Bates_2010_lme4_book.rsy
             random   <- VarCorr( lmerObj )
             fixed    <- fixef(lmerObj)
     ### Sektion 'random effects einlesen'
             randomF  <- do.call("rbind", lapply(names(random), FUN = function ( y ) {
                         ret <- data.frame(model = model, Var1 = colnames(random[[y]]), Var2 = NA, type = "random", group = y, par = "var", derived.par = NA, value = Matrix::diag(random[[y]]), stringsAsFactors = FALSE )
                         ret <- rbind(ret, data.frame(model = model, Var1 = colnames(random[[y]]), Var2 = NA, type = "random", group = y, par = "sd", derived.par = NA, value = sqrt(Matrix::diag(random[[y]])), stringsAsFactors = FALSE ))
                         if(nrow(attr(random[[y]], "correlation")) > 1)  {      ### Gibt es Korrelationen?
                            ...
                            ret      <- plyr::rbind.fill( ret, data.frame ( model=model, korTab[intersect(wahl1, wahl2),], type = "random", group = y, par = "correlation", derived.par = NA, stringsAsFactors = FALSE ) )
                         }
                         return(ret)}))
             if ( !is.na(attr(random, "sc"))) {                                 ### vergleichen zwischen Versionen!
                  randomF  <- plyr::rbind.fill ( randomF, rbind ( data.frame ( model=model, Var1 = "residual", type = "random", par = "var", derived.par = NA, value = attr(random, "sc")^2, stringsAsFactors = FALSE ), data.frame ( model=model, Var1 = "residual", type = "random", par = "sd", derived.par = NA, value = attr(random, "sc"), stringsAsFactors = FALSE )))
     ...
     ### Sektion 'fixed effects einlesen'
             ...
             fixedF   <- rbind(fixedF, data.frame ( model=model, Var1 = names(fixed), Var2 = NA, type = "fixed", group = NA, par = "se", derived.par = NA, value = sqrt(Matrix::diag(vcov(lmerObj))), stringsAsFactors = FALSE ))
             ...
             if(nrow( korMat ) > 1 ) {
                 ...
                 wahl1    <- which(!duplicated(apply(korTab, 1, FUN = function ( xx ) { paste( sort(c(xx[1], xx[2])), collapse="_") })))
                 fixedF   <- plyr::rbind.fill( fixedF, data.frame ( model=model, korTab[intersect(wahl1, wahl2),], type = "fixed", group = NA, par = "correlation", derived.par=NA, stringsAsFactors = FALSE ) )
             }
     ...
     ### Berechnung des r^2: eigentlich mit folgender etwas umstaendlicher methode. Die klappt allerdings nicht, wenn lmerTest geladen wurde. Deshalb MuMIn. Das gibt allerdings manchmal Arbeitsspeicherprobleme. Also beides implementieren ...
     ### wenn quick = TRUE; dann soll NICHT MuMIn verwendet werden
             if(quick || inherits(try(r2 <- MuMIn::r.squaredGLMM(lmerObj)  ),"try-error"))  {
                  err      <- NA                                                ### Fehlerterm initialisieren
                  if(inherits(lmerObj, "glmerMod")) {                           ### jetzt das R^2, dazu Fehlerkomponente bestimmen, haengt von Linkfunktion ab
     ...
     ### Achtung, kompliziert: poisson-models brauchen das Nullmodell (alle random effects, keine fixed effects)
                     if ( lmerObj@resp$family$family == "poisson" & lmerObj@resp$family$link == "log") {cat("Calculation of r^2 needs estimation of the null model. Will be implemented at a later date.\n")}
                  }                                                             ### problem: was, wenn LMMs ueber "glmer" mit family=gaussian aufgerufen werden? --> ok, gibt eine lme4-Warnung
                  if(inherits(lmerObj, "lmerMod")) { err <- attr(VarCorr(lmerObj), "sc")^2 }
                  if(!is.na(err)) {                                             ### r^2 wird nur berechnet, wenn es irgendwie gelungen ist, die Fehlerkomponente zu identifizieren
                      ...
                      namesRan <- names(VarCorr(lmerObj))                       ### das klappt bspw. nicht, wenn das Paket 'lmerTest' geladen wurde. In diesem Fall wird r^2 mithilfe von MuMIn berechnet
                  ...
                  deviancF <- rbind(deviancF, data.frame(model=model, type = "model", par = c("R2_m", "R2_c"), value = c(r2[1,"R2m"], r2[1,"R2c"]), stringsAsFactors = FALSE))
             }
...
#'@param standardized Logical: Additionally print standardized coefficients to the output?
#'@param digits Number of digits for printing standardized results
#'@param quick Logical: Sometimes, default method for computing \eqn{R^2} by calling \code{\link[MuMIn]{r.squaredGLMM}} is very time-consuming. Using \code{quick = TRUE} forces the function to choose an alternative method which, however, is not available when \code{lmerTest} is attached. If \code{lmerTest} is attached, no \eqn{R^2} is computed if \code{quick = TRUE}.
#'
...
#'}
#'@export
save.lmer.effects <- function ( lmerObj, lmerObjRestrict = NULL, fileName, scipen=6, standardized = FALSE, digits = 3, quick = FALSE) {
           ret   <- get.lmer.effects(lmerObj, saveData = FALSE, quick=quick)
     ...
     ### Sektion standardized coefficients abspeichern (falls gewollt)
           if ( isTRUE(standardized) ) {
                cat("\nStandardized coefficients:\n")
                txt <- capture.output( print(parameters::model_parameters(lmerObj, standardize = "smart"), digits = 3))
                cat(txt[-1], sep="\n")
                cat("\n")
           }
     ...
     ### Sektion 'Modellvergleich' (optional) ... auch hier das R^2 abspeichern!
           if(!is.null(lmerObjRestrict)) {
               retR  <- get.lmer.effects(lmerObjRestrict, saveData = FALSE, quick=quick)
               cat("\n\nH0 model:\n\n")
               print(summary(lmerObjRestrict))
               if ( isTRUE(standardized) ) {
                    cat("\nStandardized coefficients:\n")
                    txt <- capture.output( print(parameters::model_parameters(lmerObjRestrict, standardize = "smart"), digits = 3))
                    cat(txt[-1], sep="\n")
                    cat("\n")
               }
